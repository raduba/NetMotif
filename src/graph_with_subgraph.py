...
from src.progress import ProgressUpdate, Logger
...
class GraphWithSubgraph(Graph):
    def __init__(self, graph_type, input, motif_size, progress: ProgressUpdate = None, logger: Logger = None):
        ...
        self.runESU(motif_size, graph_type, progress, logger)
        esu_time = time.perf_counter()
        if logger:
            logger(f"runESU time: {(esu_time - start_time):.6f} seconds")
        # end_time = time.time()
        # st.write(f"Time to run ESU and enumerate subgraphs: {end_time - start_time:.2f} seconds")

    def runESU(self, motif_size, graph_type, progress: ProgressUpdate, logger: Logger):
        # produce list of subgraphs
        self.esu = ESU(self.G, motif_size, graph_type, progress, logger)
